    ...
    <div class="profile-container">
       <nav class="profile-navbar py-4">
        ...
        <div class="container mx-auto px-4 md:px-6 py-8 max-w-6xl">
            ...
            {{-- Admin: only pfp, email, and password --}}
            <div class="space-y-6">

                {{-- Profile Picture + Email --}}
                <div class="p-6 md:p-7 profile-card">
                    <h2 class="section-title"><i class="fas fa-user-shield"></i> Admin Profile</h2>

                    {{-- Profile Picture --}}
                    <form method="POST" action="{{ route('profile.admin.picture') }}" enctype="multipart/form-data" class="mb-8">
                        @csrf
                        <div class="flex flex-col sm:flex-row items-center gap-6">
                            <div class="relative flex-shrink-0">
                                <div class="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-lg bg-[#f6f1ea]">
                                    @if(Auth::user()->profile_picture)
                                        <img src="{{ asset('storage/' . Auth::user()->profile_picture) }}"
                                             ...
                                             id="pfp-preview">
                                    @else
                                        <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#8f1d1d] to-[#5b0f0f]" id="pfp-placeholder">
                                            <i class="fas fa-user-shield text-white text-3xl"></i>
                                        </div>
                                        <img src="" alt="" class="w-full h-full object-cover hidden" id="pfp-preview">
                                    @endif
                                </div>
                                <label for="profile_picture" class="absolute bottom-0 right-0 w-7 h-7 bg-[#8f1d1d] rounded-full flex items-center justify-center cursor-pointer shadow border-2 border-white hover:bg-[#6f1414] transition">
                            ...
                            <div class="flex-1">
                                <p class="label-text mb-1">Profile Photo</p>
                                <p class="helper-text mb-3">JPG, PNG or GIF · Max 4MB</p>
                                <button type="submit" class="btn-primary text-sm px-4 py-2">
                                    <i class="fas fa-upload mr-1.5"></i> Upload Photo
                                </button>
                                @if(session('status') === 'admin-picture-updated')
                                    <p class="text-green-600 text-sm mt-2"><i class="fas fa-check-circle mr-1"></i> Photo updated.</p>
                                @endif
                            </div>
                        </div>
                        @error('profile_picture') <p class="error-text mt-2">{{ $message }}</p> @enderror
                    </form>
                    ...
                    <form method="POST" action="{{ route('profile.admin.update') }}">
                        ...
                        @method('patch')
                        <div class="max-w-md">
                            <label for="admin_email" class="block label-text">Email Address</label>
                            ...
                            @if(session('status') === 'admin-profile-updated')
                                <p class="text-green-600 text-sm mt-2"><i class="fas fa-check-circle mr-1"></i> Email updated.</p>
                            @endif
                            <div class="mt-5">
                                <button type="submit" class="btn-primary">
                                    <i class="fas fa-save mr-1.5"></i> Save Email
                                </button>
                            </div>
                        </div>
                    </form>
                ...
                <div class="p-6 md:p-7 profile-card">
                    ...
                    <form method="POST" action="{{ route('profile.password.update') }}">
                        ...
                        <div class="space-y-5 max-w-md">
                            <div>
                                <label for="admin_current_password" class="block label-text">Current Password</label>
                                <input type="password" id="admin_current_password" name="current_password" class="form-input">
                                @error('current_password') <p class="error-text">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="admin_new_password" class="block label-text">New Password</label>
                                <input type="password" id="admin_new_password" name="password" class="form-input">
                                @error('password') <p class="error-text">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="admin_password_confirmation" class="block label-text">Confirm New Password</label>
                                <input type="password" id="admin_password_confirmation" name="password_confirmation" class="form-input">
                            </div>
                            @if(session('status') === 'password-updated')
                                <p class="text-green-600 text-sm"><i class="fas fa-check-circle mr-1"></i> Password updated.</p>
                            @endif
                            <div>
                                <button type="submit" class="btn-primary">
