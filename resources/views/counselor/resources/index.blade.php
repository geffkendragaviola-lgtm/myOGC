...
<div class="min-h-screen resources-shell">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 py-5 md:py-8">
        ...
        <!-- Filter Card -->
        <div class="panel-card mb-6 sm:mb-8 overflow-hidden relative">
            <div class="panel-topline z-10"></div>
            <div class="p-4 sm:p-5">
                <form method="GET" action="{{ route('counselor.resources.index') }}" class="flex flex-wrap items-end gap-3">
                    <div class="flex-1 min-w-[140px]">
                        <label class="filter-label">Search</label>
                        <div class="relative">
                            <i class="fas fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-[#a89f97] text-xs"></i>
                            <input type="text" name="search" value="{{ request('search', $search ?? '') }}" placeholder="Title, description..." class="filter-input" style="padding-left: 2.25rem !important;" />
                        </div>
                    </div>
                    ...
                    <div class="min-w-[120px]">
                        ...
                        <select name="category" class="filter-input bg-white">
                            <option value="all" {{ request('category', $category ?? 'all') === 'all' ? 'selected' : '' }}>All</option>
                            @foreach(($categories ?? []) as $key => $label)
                                <option value="{{ $key }}" {{ request('category', $category ?? '') === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="min-w-[110px]">
                        ...
                        <select name="status" class="filter-input bg-white">
                            <option value="all" {{ request('status', $status ?? 'all') === 'all' ? 'selected' : '' }}>All</option>
                    ...
                    <div class="min-w-[110px]">
                        ...
                        <select name="pinned" class="filter-input bg-white">
                            <option value="all" {{ request('pinned', $pinned ?? 'all') === 'all' ? 'selected' : '' }}>All</option>
                            <option value="pinned" {{ request('pinned', $pinned ?? '') === 'pinned' ? 'selected' : '' }}>Pinned</option>
                            <option value="unpinned" {{ request('pinned', $pinned ?? '') === 'unpinned' ? 'selected' : '' }}>Unpinned</option>
                        </select>
                    </div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('counselor.resources.index') }}" class="secondary-btn px-4 py-2 text-xs sm:text-sm rounded-lg text-center">
            ...
            <div class="panel-header">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        @if(method_exists($resources, 'total') && $resources->total() > 0)
                        ...
                        @elseif(count($resources) > 0)
                            <p class="text-[10px] sm:text-xs text-[#8b7e76]">Showing <span class="font-bold text-[#2c2420]">{{ count($resources) }}</span> resources</p>
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <div class="resource-card flex flex-col h-full {{ $resource->is_pinned ? 'ring-2 ring-[#d4af37]/40 ring-offset-2 ring-offset-[#faf8f5]' : '' }}">
                        ...
                        <div class="p-4 sm:p-5 flex flex-col flex-1">
                            ...
                            <div class="grid grid-cols-2 gap-2 mt-auto">
                                ...
                                <button onclick="togglePin({{ $resource->id }}, this)"
                                ...
                                <form action="{{ route('counselor.resources.update-status', $resource) }}" method="POST" class="contents status-toggle-form">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <form action="{{ route('counselor.resources.destroy', $resource) }}" method="POST" class="contents" onsubmit="return confirm('Are you sure you want to delete this resource?')">
                                    @csrf
                                    @method('DELETE')
...
<script>
    ...
    function togglePin(id, btn) {
        const url = `/counselor/resources/${id}/pin`;
