...
<style>
    ...
    .hero-card, .panel-card, .section-card, .status-card {
        position: relative; overflow: hidden; border-radius: 0.75rem;
        border: 1px solid var(--border-soft); background: rgba(255,255,255,0.95);
        backdrop-filter: blur(8px); box-shadow: 0 2px 8px rgba(44,36,32,0.04);
        transition: box-shadow 0.2s ease;
    }
    .hero-card:hover, .panel-card:hover, .section-card:hover, .status-card:hover { 
        box-shadow: 0 4px 14px rgba(44,36,32,0.06); 
    }
    .hero-card::before, .panel-card::before, .section-card::before, .status-card::before {
    ...
    .panel-header, .section-header { display: flex; align-items: center; gap: 0.7rem; padding: 0.85rem 1.25rem; border-bottom: 1px solid var(--border-soft)/60; }
    .panel-icon, .section-icon { width: 2rem; height: 2rem; border-radius: 0.6rem; display: flex; align-items: center; justify-content: center; background: rgba(254,249,231,0.7); color: var(--maroon-700); }
    ...
    .textarea-field { padding: 0.65rem 0.75rem; resize: vertical; min-height: 120px; line-height: 1.5; }
    ...
    .option-card { background: rgba(250,248,245,0.7); border: 1px solid var(--border-soft); padding: 0.7rem 0.85rem; border-radius: 0.65rem; }
    .option-card:hover { border-color: rgba(212,175,55,0.4); background: rgba(254,249,231,0.5); }
    .radio-option input, .checkbox-option input, .option-card input[type="checkbox"] { margin-top: 0.15rem; width: 1rem; height: 1rem; accent-color: var(--maroon-700); cursor: pointer; flex-shrink: 0; }
    .radio-option label, .checkbox-option label, .option-card label { font-size: 0.8rem; color: var(--text-secondary); cursor: pointer; line-height: 1.4; }
...
<div class="min-h-screen announce-edit-shell">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 py-5 md:py-8">
                    ...
                    <div class="section-card mb-4">
                        ...
                        <div class="section-header">
                            ...
                            <div>
                                ...
                                <p class="section-subtitle hidden sm:block">Update the message and optional visual cover for the announcement.</p>
                            </div>
                        </div>
        ...
        @if($announcement->image_url)
        <form action="{{ route('counselor.announcements.remove-image', $announcement) }}" method="POST" id="remove-image-form" class="hidden">
            @csrf
            @method('DELETE')
        </form>
        @endif
